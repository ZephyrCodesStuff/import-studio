...
namespace Import.Viewers {
    public class ChainViewer: UserControl, ISelectParentViewer, IDroppable {
        ...
        void ContextMenu_Action(ImportContextMenu sender, string action) =>
            Device_Action(action, sender == (ImportContextMenu)this.Resources["DeviceContextMenuAfter"]);

        void Click(object sender, PointerReleasedEventArgs e) {
            PointerUpdateKind MouseButton = e.GetCurrentPoint(this).Properties.PointerUpdateKind;

            if (MouseButton == PointerUpdateKind.RightButtonReleased)
                if (sender == DropZoneBefore) ((ImportContextMenu)this.Resources["DeviceContextMenuBefore"]).Open((Control)sender);
                else if (sender == DropZoneAfter) ((ImportContextMenu)this.Resources["DeviceContextMenuAfter"]).Open((Control)sender);
