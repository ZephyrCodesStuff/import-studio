...
namespace Import.Components {
    public class DeviceAdd: AddButton {
        ...
        void AddContextMenu_Action(string action) => Added?.Invoke(Assembly.GetExecutingAssembly().GetType($"Import.Devices.{action.Replace(" ", "")}"));
        ...
        protected override void Click(PointerReleasedEventArgs e) {
            ...
            if (MouseButton == PointerUpdateKind.LeftButtonReleased) ((ImportContextMenu)this.Resources["AddContextMenu"]).Open(Icon);
            else if (MouseButton == PointerUpdateKind.RightButtonReleased) ((ImportContextMenu)this.Resources["DeviceContextMenu"]).Open(Icon);
        }
    }
}
