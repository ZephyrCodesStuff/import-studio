    ...
    public class SplashWindow: Window {
        ...
        async void UpdateBlogpost() {
            ...
            BlogpostBody.Text = $"{latest.Content.Replace("\r", "").Split('\n').First().Replace("# ", "").Replace("#", "")}\n" +
                $" published {DateTimeOffset.FromUnixTimeSeconds(Convert.ToInt64(Path.GetFileNameWithoutExtension(latest.Name)))}";
        ...
        async void UpdateRelease() {
            ...
            ReleaseVersion.Text = $"{latest.Name} - published {latest.PublishedAt}";
        ...
        async void CheckUpdate() {
            if (await Github.ShouldUpdate()) {
                ...
                Dispatcher.UIThread.Post(async () => {
                    UpdateButton.Enable($"Updates are available for Import Studio ({(await Github.LatestRelease()).Name} - {(await Github.LatestDownload()).Size.Bytes()}).");
        ...
        async void Blogpost(object sender, PointerReleasedEventArgs e)
            => App.URL($"https://import.mat1jaczyyy.com/post/{Path.GetFileNameWithoutExtension((await Github.LatestBlogpost()).Name)}");
