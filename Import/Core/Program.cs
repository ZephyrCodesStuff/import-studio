...
namespace Import.Core {
    class Program {
        public static readonly string Version = "Version 1.8.13";
        ...
        static void Main(string[] args) {
            ...
            if (LaunchAdmin && RuntimeInformation.IsOSPlatform(OSPlatform.Windows))
                Process.Start(
                    ...
                    $"\"{Path.Combine(Program.GetBaseFolder("Import"), "Import.exe")}\" --update"
                );
                ...
                if (RuntimeInformation.IsOSPlatform(OSPlatform.Windows))
                    Process.Start(
                        $"{AppDomain.CurrentDomain.BaseDirectory}elevate.exe",
                        $"\"{Path.Combine(Program.GetBaseFolder("Update"), "ImportUpdate.exe")}\""
                    );
                
                else if (RuntimeInformation.IsOSPlatform(OSPlatform.OSX))
                    Process.Start(Path.Combine(Program.GetBaseFolder("Update"), "ImportUpdate"));
            }
        }
    }
}
